...
import type { PMProjectStatus } from "@/lib/validations/project"
...
const COLUMN_ORDER: PMProjectStatus[] = ["BACKLOG", "PLANNED", "ACTIVE", "COMPLETED"]

const COLUMN_LABELS: Record<PMProjectStatus, string> = {
  BACKLOG: "Backlog",
  PLANNED: "Planned",
  ACTIVE: "Active",
  COMPLETED: "Completed",
...
function columnStatusIcon(status: PMProjectStatus): React.JSX.Element {
  switch (status) {
    case "BACKLOG":
...
export function ProjectBoardView({ projects, loading = false, onAddProject }: ProjectBoardViewProps) {
  ...
  const groups = useMemo(() => {
    const m = new Map<PMProjectStatus, ProjectListItem[]>()
    for (const s of COLUMN_ORDER) m.set(s, [])
    for (const p of items) {
      const col = m.get(p.status)
      if (col) col.push(p)
    }
    return m
  }, [items])

  const onDropTo = (status: PMProjectStatus) => (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault()
  ...
  const draggableCard = (p: ProjectListItem) => (
    <div
      key={p.id}
      draggable
