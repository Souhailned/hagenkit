...
function GenericPicker<T extends { id: string; label?: string; name?: string }>({
  trigger,
  items,
  onSelect,
  selectedId,
...
function DatePicker({ date, onSelect, trigger }: DatePickerProps) {
  const [open, setOpen] = useState(false)
...
export function StepQuickCreate({ onClose, onCreate, onExpandChange }: StepQuickCreateProps) {
  ...
  const [lead, setLead] = useState<(typeof USERS)[0] | null>(null)
  ...
  const buildProjectData = (): QuickCreateData => ({
    name: title || "Untitled Project",
    description: description || undefined,
    priority: priority?.id === "no-priority" ? undefined : priority?.id,
    status: status.id === "backlog" ? "backlog" : status.id === "todo" ? "planned" : status.id === "in-progress" ? "active" : status.id === "done" ? "completed" : status.id === "canceled" ? "cancelled" : "backlog",
    startDate: startDate ? startDate.toISOString() : undefined,
    endDate: targetDate ? targetDate.toISOString() : undefined,
    typeLabel: sprintType?.label,
  })

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if ((e.metaKey || e.ctrlKey) && e.key === "Enter") {
      onCreate(buildProjectData())
    }
  }
      ...
      <div className="flex flex-col flex-1 p-3.5 px-4 gap-3.5 overflow-hidden">
        ...
        <div className="flex flex-wrap gap-2.5 items-start w-full shrink-0">
            ...
            trigger={
              <button
                className={cn(
                  ...
                  lead ? "bg-muted" : "bg-background hover:bg-black/5"
                )}
              >
                <UserCircle className="size-4 text-muted-foreground" />
                <span className="font-medium text-foreground text-sm leading-5">{lead ? lead.name : "Lead"}</span>
              </button>
            }
          />
          ...
          <GenericPicker
            ...
            trigger={
              <button className="flex gap-2 h-9 items-center px-3 py-2 rounded-lg border border-border transition-colors bg-background hover:bg-black/5">
                <Spinner className="size-4 text-muted-foreground" />
                {status.id !== "backlog" && <div className={cn("size-2 rounded-full", status.dotClass)} />}
        ...
        <div className="flex items-center justify-between mt-auto w-full pt-4 shrink-0">
          ...
          <button
            onClick={() => onCreate(buildProjectData())}
          ...
          >
            <span className="font-medium text-primary-foreground text-sm leading-5">Create Project</span>
          </button>
        </div>
      </div>
    </div>
