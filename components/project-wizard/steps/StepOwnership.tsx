...
import type { ProjectData, OwnershipEntry } from "../types"
...
const DEFAULT_ACCOUNTS: Account[] = [
  { id: "jason-d", name: "Jason D", email: "jason.duong@mail.com", team: "Product", initials: "JD" },
  { id: "alex-morgan", name: "Alex Morgan", email: "alex.morgan@workspace.com", team: "Product", initials: "AM" },
  { id: "sarah-chen", name: "Sarah Chen", email: "sarah.chen@workspace.com", team: "Engineering", initials: "SC" },
  { id: "mike-ross", name: "Mike Ross", email: "mike.ross@workspace.com", team: "Design", initials: "MR" },
  { id: "harrold", name: "Harrold", email: "harrold@workspace.com", team: "Engineering", initials: "H" },
  { id: "james", name: "James", email: "james.boarnd@workspace.com", team: "Product", initials: "JB" },
  { id: "mitch", name: "Mitch", email: "mitch.sato@workspace.com", team: "Design", initials: "MS" },
]
...
export function StepOwnership({ data, updateData }: StepOwnershipProps) {
  ...
  const getAccountById = (accountId: string): Account | undefined => accounts.find((a) => a.id === accountId)
  ...
  const handleAdd = (target: "contributors" | "stakeholders") => {
    ...
    let account = accounts.find(
      (a) => a.email.toLowerCase() === value.toLowerCase() || a.name.toLowerCase() === value.toLowerCase()
    )
    ...
    const nextList: OwnershipEntry[] = [...currentList, { accountId: account.id, access }]
  ...
  const handlePermissionChange = (
    ...
    const currentList = target === "contributors" ? contributorOwnerships : stakeholderOwnerships
    const nextList = currentList.map((entry) => (entry.accountId === accountId ? { ...entry, access } : entry))
    syncOwnerships(target, nextList)
  }
          ...
          <div className="flex items-center justify-between rounded-lg border-border bg-muted/40">
            <div className="flex items-center gap-3">
              <Avatar className="h-5 w-5">
                <AvatarFallback>{ownerAccount ? ownerAccount.initials : "PO"}</AvatarFallback>
              </Avatar>
              <div className="flex flex-col text-left">
                <span className="text-sm font-medium">{ownerAccount?.name ?? "Project owner"}</span>
              </div>
            </div>
