...
export function ViewOptionsPopover({ options, onChange, onSetDefault, onReset, allowedViewTypes }: ViewOptionsPopoverProps) {
        ...
        <div className="p-4">
          ...
          <div className="flex rounded-xl p-1 bg-muted">
            {viewTypes.map((type) => (
              <button
                key={type.id}
                onClick={() => onChange({ ...options, viewType: type.id as ViewOptions['viewType'] })}
                className={cn(
                ...
                <PopoverContent className="w-40 rounded-xl p-1" align="end">
                  {taskOptions.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => {
                        onChange({ ...options, tasks: option.id as ViewOptions['tasks'] })
                        setTasksOpen(false)
                      }}
                      className={cn(
                ...
                <PopoverContent className="w-36 rounded-xl p-1" align="end">
                  {orderingOptions.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => {
                        onChange({ ...options, ordering: option.id as ViewOptions['ordering'] })
                ...
                <PopoverContent className="w-44 rounded-xl p-1" align="end">
                  {groupByOptions.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => {
                        onChange({ ...options, groupBy: option.id as ViewOptions['groupBy'] })
          ...
          <div className="mt-4 flex items-center justify-between border-t border-border/40 pt-4">
            ...
            <button
              onClick={() => onReset ? onReset() : onChange(DEFAULT_VIEW_OPTIONS)}
