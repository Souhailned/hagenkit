---
title: Configure DataRAG
updatedAt: 2025-05-09
summary: Connect Azure Active Directory to DataRAG so your team signs in with existing corporate credentials.
author: DataRAG Team
categories:
  - analysis
slug: azure-saml-sso
...
<Note variant="success">
  SAML single sign-on is available on DataRAG Scale and Enterprise plans.
</Note>

DataRAG Scale customers can enforce single sign-on through Azure Active Directory using the SAML 2.0 protocol. The steps below walk through the Azure configuration and the fields DataRAG expects.
...
In the next screen, click on **Create your own application**. Give your application a **Name** (e.g. "DataRAG") and click **Create**.
...
<CopyBox title="Identifier (Entity ID)" copy="https://sso.datarag.io" />

<CopyBox
  title="Reply URL (Assertion Consumer Service URL)"
  copy="https://api.datarag.io/auth/saml/callback"
/>
...
Scroll down to the 3rd section on the page, **SAML Certificates**. Copy the **App Federation Metadata Url** value—DataRAG uses it to keep your certificate in sync.
...
## Step 5: Configure SAML SSO on DataRAG

In your DataRAG workspace, open **Settings → Security**.

<Image
  alt="SAML SSO section on the DataRAG dashboard"
...
Under **SAML Single Sign-On**, click **Configure**. This opens the DataRAG SSO modal:
...
Your assigned users should now receive an invitation email to join your DataRAG workspace.
...
<Note variant="warning">
  ...
  invitation email and be able to join your DataRAG workspace.
</Note>

They will also be able to sign in to DataRAG using Azure AD SSO.
