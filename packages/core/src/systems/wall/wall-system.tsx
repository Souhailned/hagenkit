...
export const WallSystem = () => {
  useFrame(() => {
    // Read dirtyNodes from getState() instead of a selector closure.
    // Selectors in R3F's reconciler can lag behind by a frame after
    // setScene(), causing the closure to reference an older (empty) Set
    // while the store already holds the newly-populated one.
    const { dirtyNodes, clearDirty, nodes } = useScene.getState()
