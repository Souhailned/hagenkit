...
const useScene: UseSceneStore = create<SceneState>()(
  temporal(
    (set, get) => ({
      ...
      loadScene: () => {
        if ((get().rootNodeIds?.length ?? 0) > 0) {
          // Assign all nodes as dirty to force re-validation via a single
          // set() call so subscribers get the populated Set immediately.
          const allDirty = new Set<AnyNodeId>(
            Object.values(get().nodes).map((node) => node.id),
          )
          set({ dirtyNodes: allDirty })
